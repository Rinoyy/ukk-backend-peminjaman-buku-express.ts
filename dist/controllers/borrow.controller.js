...
exports.getFinesRecap = exports.getBorrowings = exports.payFine = exports.handleReturnRequest = exports.markPickedUp = exports.returnBookRequest = exports.handleBorrowRequest = exports.cancelBorrow = exports.borrowBook = exports.getMyFines = exports.checkEligibility = void 0;
...
const checkEligibility = async (req, res) => {
    ...
    try {
        ...
        const unreturnedBook = await prisma_1.default.borrowing.findFirst({
            where: {
                userId,
        ...
        const unpaidFine = await prisma_1.default.borrowing.findFirst({
            where: {
                userId,
                totalFine: { gt: 0 },
                isPaid: false,
...
const getMyFines = async (req, res) => {
    ...
    if (!userId)
        return res.status(401).json({ message: "Unauthorized" });
    try {
        const fines = await prisma_1.default.borrowing.findMany({
            where: {
                userId,
                totalFine: { gt: 0 },
                isPaid: false
...
const borrowBook = async (req, res) => {
    ...
    try {
        ...
        // Find available copy
        const availableCopy = await prisma_1.default.bookCopy.findFirst({
            where: {
                bookId,
                status: 'AVAILABLE'
            }
        ...
        const borrowing = await prisma_1.default.borrowing.create({
            data: {
                userId,
...
const cancelBorrow = async (req, res) => {
    ...
    if (!userId)
        return res.status(401).json({ message: "Unauthorized" });
    try {
        const borrowing = await prisma_1.default.borrowing.findUnique({
            where: { id: Number(id) },
...
exports.handleBorrowRequest = handleBorrowRequest;
/**
...
const returnBookRequest = async (req, res) => {
    ...
    const userId = req.user?.userId;
    try {
        const borrowing = await prisma_1.default.borrowing.findUnique({ where: { id: Number(id) } });
        if (!borrowing)
            return res.status(404).json({ message: "Borrowing not found" });
        if (borrowing.userId !== userId)
            return res.status(403).json({ message: "Unauthorized" });
        ...
        const updated = await prisma_1.default.borrowing.update({
            where: { id: Number(id) },
...
exports.markPickedUp = markPickedUp;
/**
...
const getBorrowings = async (req, res) => {
    ...
    try {
        ...
        if (role === 'ADMIN' || role === 'PETUGAS') {
            borrowings = await prisma_1.default.borrowing.findMany({
                include: {
                    user: { select: { username: true } },
                    bookCopy: {
 ...
 */
const getFinesRecap = async (req, res) => {
    // Only Admin/Petugas
    const role = req.user?.role;
    if (role !== 'ADMIN' && role !== 'PETUGAS') {
        return res.status(403).json({ message: "Unauthorized" });
    ...
    try {
        // 1. Calculate Summary
        const paidFines = await prisma_1.default.borrowing.aggregate({
            where: { isPaid: true, totalFine: { gt: 0 } },
            _sum: { totalFine: true },
            _count: { id: true }
        });
        const unpaidFines = await prisma_1.default.borrowing.aggregate({
            where: { isPaid: false, totalFine: { gt: 0 } },
            _sum: { totalFine: true },
            _count: { id: true }
        });
        // 2. Get List of Fines (Both paid and unpaid)
        const finesList = await prisma_1.default.borrowing.findMany({
...
exports.getFinesRecap = getFinesRecap;
